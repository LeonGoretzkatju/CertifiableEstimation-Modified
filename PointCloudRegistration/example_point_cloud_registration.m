...
%% draw initial two point cloud
origin_bunnyxyz_t = problem.R_gt * bunnyxyz + problem.t_gt;
origin_bunnypcd_t = pointCloud(origin_bunnyxyz_t');
figure;
pcshowpair(bunnypcd,origin_bunnypcd_t);
hold on;
...
%     X0       = rank_one_lift(v);
    ...
    aligned_bunnyxyz = solution.R_est * bunnyxyz + solution.t_est;
    aligned_bunnyPCD = pointCloud(aligned_bunnyxyz');
    figure;
    pcshowpair(origin_bunnypcd_t,aligned_bunnyPCD);
...
% title("after certifiable registration")